    ...
    return (
      <div className="space-y-6">
        {actions.map((action: Action, i: any) => (
          <div
            key={i}
            className="w-full py-4 space-y-2 lg:py-0 relative cursor-pointer rounded-xl border h-fit lg:h-20 px-4 lg:px-10 flex flex-wrap lg:flex-nowrap items-center justify-between overflow-x-hidden"
          >
            {!is_loaded && (
            ...
            {action.is_error ? (
              <Badge variant="destructive" className="flex gap-2">
                <span>Error al convertir el archivo</span>
                <BiError />
              </Badge>
            ) : action.is_converted ? (
  ...
  return (
    <ReactDropzone
      ...
      onDropRejected={() => {
        ...
        toast({
          variant: 'destructive',
          title: 'Error cargando tu(s) archivo(s)',
          description: 'Archivos permitidos: Audio, Video e Imagenes.',
          duration: 5000,
        });
      }}
      onError={() => {
